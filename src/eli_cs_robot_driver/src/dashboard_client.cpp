...
{
  // SDK 枚举：UNKNOWN=0, PLAYING=1, PAUSED=2, STOPPED=3
  // ROS 枚举：UNKNOWN=-1, STOPPED=0, PAUSED=1, PLAYING=2
  switch (sdk_status) {
...
DashboardClient::DashboardClient(const rclcpp::NodeOptions& options)
: Node("dashboard_client", options)
{
  ...
  shutdown_service_ = createTriggerService("~/shutdown", [&]() -> bool {
    client_.shutdown();
    return true;
  });
  reboot_service_ = createTriggerService("~/reboot", [&]() -> bool {
    client_.reboot();
    return true;
  });
  ...
  quit_service_ = createTriggerService("~/quit", [&]() -> bool {
    client_.quit();
    return true;
  });

  safety_system_restart_service_ =
    createTriggerService("~/restart_safety", [&]() -> bool { return client_.safetySystemRestart(); });
  ...
  add_log_service_ = this->create_service<eli_dashboard_interface::srv::Log>(
    ...
    [&](const eli_dashboard_interface::srv::Log::Request::SharedPtr req,
      ...
      try {
        bool ok = client_.log(req->message);

        resp->success = ok;
        if (ok) {
          // 这里用中文或英文都可以，看你面板那边怎么展示
          resp->message = "Log sent to robot successfully";
        } else {
          resp->message = "Robot rejected log request or returned failure";
        }
      } catch (const ELITE::EliteException& e) {
        resp->success = false;
        resp->message = e.what();
  ...
  task_status_service_ = this->create_service<eli_common_interface::srv::GetTaskStatus>(
    ...
    [&](const eli_common_interface::srv::GetTaskStatus::Request::SharedPtr req,
      ...
      (void)req;
      try {
        ELITE::TaskStatus sdk_status = client_.getTaskStatus();
        resp->status.status = toRosTaskStatus(sdk_status);
        resp->success = true;
      } catch (const ELITE::EliteException& e) {
        resp->success = false;
        resp->message = e.what();
        // 出错时也返回一个明确状态
        resp->status.status = eli_common_interface::msg::TaskStatus::UNKNOWN;
      }
    });
  ...
  load_configure_service_ = this->create_service<eli_dashboard_interface::srv::Load>(
    "~/load_configure",
    [&](const eli_dashboard_interface::srv::Load::Request::SharedPtr req,
        eli_dashboard_interface::srv::Load::Response::SharedPtr resp) {
      try {
        RCLCPP_INFO(this->get_logger(),
                    "Calling SDK loadConfiguration with filename: [%s]",
                    req->filename.c_str());

        bool ret = client_.loadConfiguration(req->filename);

        RCLCPP_INFO(this->get_logger(),
                    "SDK loadConfiguration returned: %s",
                    ret ? "true" : "false");
        ...
        RCLCPP_ERROR(this->get_logger(),
                     "SDK loadConfiguration threw exception: %s",
                     e.what());
        resp->success = false;
        resp->answer = e.what();
      }
  ...
  custom_request_service_ = this->create_service<eli_dashboard_interface::srv::CustomRequest>(
    "~/custom_request",
    [&](const eli_dashboard_interface::srv::CustomRequest::Request::SharedPtr req,
